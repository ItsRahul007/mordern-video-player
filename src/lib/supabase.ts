...
/**
 * Fetch recorded app-opens, newest first.
 *
 * When `sinceIso` is given, only rows with an `opened_at` at or after that
 * instant are returned (legacy rows with a null `opened_at` are excluded from a
 * date-bounded query — they can't be placed on the timeline anyway). When
 * `deviceId` is given, only that install's opens are returned; omit it to fetch
 * every install's opens.
 */
export async function fetchAppOpens(
  sinceIso?: string,
  deviceId?: string,
): Promise<OpenCount[]> {
  if (!configured) {
  ...
  const res = await fetch(
    `${SUPABASE_URL}${TABLE}?select=id,opened_at,created_at&order=opened_at.desc.nullslast${since}${device}`,
    { headers: headers() },
  );
  if (!res.ok) {
